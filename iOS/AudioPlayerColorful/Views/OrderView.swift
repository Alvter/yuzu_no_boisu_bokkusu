...
struct OrderView:View {
    ...
    @ObservedObject var viewModel: OrderViewModel // 使用 @ObservedObject
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            LazyVGrid(columns: columns, spacing: 15) {
                                ForEach(group.items) { item in
                                    ...
                                    .environmentObject(viewModel) // 将 viewModel 传递给 AudioCheckbox
                                }
                            }
                            .padding()
                            .background(
                            ...
                            LazyVGrid(columns: [GridItem(.fixed(170), spacing: 10), GridItem(.fixed(170), spacing: 10)], spacing: 15) {
                                ForEach(baoPaiGroup.items) { item in
                                    HStack(spacing: 10) {
                                        ...
                                        .frame(width: 80, height: 60)
                                        .background(colorScheme == .dark ? Color.secondary : Color.white) // 根据配色方案设置背景色
                                        .foregroundColor(colorScheme == .dark ? .white : .black) // 根据配色方案设置文字颜色
                                        .cornerRadius(12)
                                        .shadow(radius: 3)
                                        .disabled(!selectedFiles.contains(item.filename))
                                        .opacity(selectedFiles.contains(item.filename) ? 1 : 0.5)
            ...
            .toolbar {
                ...
                Button(action: {
                    guard let audioOrder = loadAudioOrder() else {
                        ...
                        audioPlayer.startPlaybackQueue(for: sortedFiles, roleName: selectedRole?.filenamePrefix ?? "yiji")
                        return
                    }
        ...
        .sheet(isPresented: $viewModel.showingEasterEgg) {
            EggView(audioPlayer: audioPlayer)
        }
    }
}
